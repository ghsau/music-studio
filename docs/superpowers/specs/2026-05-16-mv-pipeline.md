...
> 2026-05-16 起探索。空镜 MV 之外的另一种形式：即梦「数字人/对口型」模式让一个生成的歌手形象对口型演唱。

**已验证**：
- 即梦数字人对口型（底层 OmniHuman）质量过关——盼长大 15s 试片对口型/形象/环境稳定，主理人确认效果好
- **卡拉OK字幕**已并进 `align-lyrics.py`（`--karaoke`）：逐字扫光 + 入场动效 + 位置上移，POC 经主理人确认

**待解决（并进 `/make-mv` 演唱模式前必须）**：
- 形象图要出 **9:16**（试片用的是 3:4，抖音竖屏会裁/留边）
- 即梦视频水印去除方案（`remove-watermark.py` 只擦 Gemini 图水印，不适用即梦视频水印）
- 逐字时间码目前是行级插值，够用；接 Suno word-level 数据是后续精度升级（非阻塞）

两个待解决项清掉后，老白再把「演唱模式」作为变体正式并进 `/make-mv`（与「空镜模式」并列）。
