...
> ⚠️ **已被 `2026-05-17-video-subsystem.md` 取代**（2026-05-17）。本文档的「完整 MV」并入新 spec 的路径 A，「演唱模式」并入路径 B。保留作历史记录，现行设计以 video-subsystem spec 为准。
...
- 卡拉OK字幕逐字时间码：**行级时长行内均分插值**。已验证（2026-05-17）——Suno `aligned_lyrics` 接口对中文是**短语/句级、非逐字**（"想要的总在身后"是一整块）；用其短语块驱动扫光实测与行级插值「区别不大」。**结论：不做 aligned 路径，卡拉OK保持纯 SRT 插值**，别再重查
