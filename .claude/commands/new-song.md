...
### 4.4 题材气质解析（2026-05-13 加，必走）

按老周 §7「步骤 1.4」做题材 attitude 解析：

1. 1-2 句话解析题材内在 attitude
2. 列 2-3 个 attitude 方向候选（参 attitude 谱）
3. **给用户拍板**（≤30 秒），选定 attitude 写到 brief 顶部新增「气质」字段
4. **绝不跳过此步直接选画像**

### 4.45 外部参照系研究（2026-05-13 加，必走）

按老周 §7「步骤 1.45」WebSearch 同类题材+attitude 榜单歌：

1. WebSearch ≥ 1 首参照歌的 BPM / 编曲 / flow / hook motif
2. 抓取关键参数（实测，不凭推论）
3. 写到 brief「参照系」字段（新增）
4. **BPM / 风格 决策必须有外部数据支撑**
